...
## CUDA的流
### CUDA流的概念
- 在CUDA中，流是由多个命令（或任务）按照顺序排成的队列。当我们将这些命令提交给一个流时，
GPU会按顺序执行这些命令。同一个流中的任务是按顺序执行的，但不同流中的任务可以并行执行。
- GPU的硬件调度器会从多个流中选择任务来执行，类似于操作系统调度器从运行队列中选择进程或线程在CPU上运行
- 每个CUDA流是一个独立的任务队列，它包含了一系列要在GPU上执行的操作。GPU硬件调度器负责管理和调度这些操作的执行

流在GPU上的执行过程
- 任务的发起：当CPU向GPU发起任务时，它会将任务（如内核执行、数据传输等）提交给一个特定的CUDA流。每个任务都会被放入相应流的“队列”中
- 任务的排序和执行：GPU上的硬件调度器会按照每个流的任务顺序执行。同一个流中的任务是顺序执行的，而不同流中的任务可以并行执行
- GPU资源分配：GPU调度器会为每个任务分配合适的计算资源（如流式多处理器SM），并确保在设备上的各个任务之间实现良好的负载均衡

### GPU架构与CUDA流并行的关系
- 流式多处理器（Streaming Multiprocessors, SMs）
  - 每个SM内部有独立的寄存器文件和共享内存，这些资源被分配给不同流中的任务。因此，一个SM可以同时为不同流中的任务提供计算支持。 
- 硬件调度器：硬件调度器负责管理流的执行，决定哪个流中的任务（如内核执行或内存传输）应该在何时执行，并将这些任务分配给相应的SM和计算资源。
  - GPU硬件调度器动态分配SM和其他硬件资源给不同的流。例如，流A中的计算任务可能被调度到SM0和SM1上执行，同时，流B中的计算任务被调度到SM2和SM3上执行。

异步执行机制
- CUDA流支持异步内存传输操作，例如cudaMemcpyAsync。这些操作可以与计算任务重叠
- GPU有专门的DMA（Direct Memory Access）引擎，用于处理内存传输。一个流中的数据传输任务可以由DMA引擎执行，而另一个流中的计算任务则在SM上执行。这两者不互相阻塞

共享资源和上下文切换
- GPU上的上下文切换（Context Switch）成本较低，因为GPU架构专门设计用于多线程并行执行。SM的硬件调度器可以快速在不同Warps之间切换，而无需保存和恢复复杂的CPU上下文
- 由于每个SM拥有大量寄存器，多个Warps的寄存器数据可以常驻在寄存器文件中，减少上下文切换时的内存访问开销
- GPU的不同硬件资源（如L1/L2缓存、寄存器文件和共享内存）可以被多个流共享。硬件调度器通过公平的策略和优先级控制在不同流之间分配这些资源，确保高效利用

调度单元和Warp调度器
- 每个SM内部有多个Warp调度器，可以同时管理多个Warp的执行。这些Warp可能来自同一个流或不同的流
- Warp调度器负责在有计算资源的情况下，优先执行计算密集型的任务

指令级并行（ILP）和多线程级并行（MTLP）
- GPU架构设计支持指令级并行（ILP）和多线程级并行（MTLP）。ILP通过硬件调度器在指令层面上寻找并行执行的机会，而MTLP通过调度不同的Warps（来自同一流或不同流）来充分利用SM的执行单元
- 硬件调度器在多个流和多个任务之间切换，最大限度地利用GPU计算资源

### CUDA流属性
CUDA流属性用于控制流的特定行为，主要包括数据访问策略和执行优先级等。以下是一些常见的CUDA流属性

1. cudaStreamAttributeAccessPolicyWindow
   - 控制流中对全局内存（global memory）的访问策略，通过设置“访问策略窗口”（Access Policy Window）来优化L2缓存行为
   - 使用场景：提高频繁访问的数据块的缓存命中率；减少不常用数据块在缓存中的停留时间。
   ```c++
   cudaStreamAttrValue stream_attribute;//定义流属性
   // base_ptr：全局内存数据指针，指向缓存策略应用的数据区域的起始地址
   stream_attribute.accessPolicyWindow.base_ptr = reinterpret_cast<void*>(ptr);
   // num_bytes：数据区域的大小，以字节为单位
   stream_attribute.accessPolicyWindow.num_bytes = num_bytes;
   // 提示预期的缓存命中率，范围在0到1之间。较高的值表示预计缓存中数据的命中率高
   stream_attribute.accessPolicyWindow.hitRatio = 0.6;
   // 缓存命中时的属性，如cudaAccessPropertyPersisting（数据持久化）
   stream_attribute.accessPolicyWindow.hitProp = cudaAccessPropertyPersisting;
   // 缓存未命中时的属性，如cudaAccessPropertyStreaming（数据流式处理）
   stream_attribute.accessPolicyWindow.missProp = cudaAccessPropertyStreaming;

   cudaStreamSetAttribute(stream, cudaStreamAttributeAccessPolicyWindow, &stream_attribute);
   ```
2.  cudaStreamAttributePriority
   - 设置流的执行优先级（priority），控制GPU硬件调度器在多个流之间的调度顺序
   ```c++
int priority;
cudaStreamGetPriority(stream, &priority);
cudaStreamAttrValue stream_attribute;
stream_attribute.priority = -1; // 设置为较高优先级
